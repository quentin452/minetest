...
local function get_sky_color(player, dtime)
	...
	if player_trans_times[player:get_player_name()] <= time then
		local current_sky_color = player:get_properties().override_day_night_ratio
		...
		if player_trans_times[player:get_player_name()] > time then
			player:set_properties({
				override_day_night_ratio = sky_color,
			})
		...
		player:set_properties({
			override_day_night_ratio = new_sky_color,
		})
