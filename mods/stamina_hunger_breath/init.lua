...
SPRINT_SPEED = 0.6		-- how much faster player can run if satiated
SPRINT_JUMP = 0.1		-- how much higher player can jump if satiated

-- global function for mods to amend stamina level (removed stamina related code)
...
function set_sprinting(player, sprinting)
   ...
   local def = {}
   -- Get player physics from 3d_armor mod (removed armor related code)
...
-- Time based sprinting function
local action_timer = 0

local function sprint_globaltimer(dtime)
...
if minetest.setting_getbool("enable_damage") and minetest.is_yes(minetest.setting_get("enable_stamina") or "1") then
   minetest.register_globalstep(sprint_globaltimer)
end
...
-- Paths for later use
local modpath = minetest.get_modpath('stamina_hunger_breath')..DIR_DELIM
